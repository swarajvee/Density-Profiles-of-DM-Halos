...
def subsample_data(file_dir,particle_range):
    X=[]
    Y=[]
    Z=[]
    N=[]
    ...
    for file_path in file_paths:
        ...
        X_cleand = ([number for array in Subsample_X for number in array if not np.isnan(number)])
        N = len(X_cleand)
    return X,Y,Z,N

x,y,z,n = subsample_data(file_dir,particle_range)
