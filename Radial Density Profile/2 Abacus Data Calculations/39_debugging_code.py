...
def rad_density_function(radius, particle_mass, b,z, part_numb):

    avg_mass=np.sum(part_numb)*particle_mass/len(part_numb)
    ...
    fraction = np.sum(part_numb)/np.sum(frequency)
...
rad_density_function(rad, particle_mass, b,z,particle_number)
