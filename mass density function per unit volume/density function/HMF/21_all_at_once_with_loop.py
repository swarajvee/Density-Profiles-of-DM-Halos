...
def halo_mass_function(z_all,model):
    for m in model:
        for z in z_all:    
            ...
            plt.plot(mf.m, mf.dndm, lw=0.5, label=f'{m} {z}')
            plt.xscale('log')
            plt.yscale('log')
...
def mass_density_fun(dir_path,z_all,l_all,b):
    for z,l in zip(z_all, l_all):
        ...
        plt.loglog()
        plt.scatter(mass_pt,density_fun,label=f'Abacus {z}',s=7)
        plt.errorbar(mass_pt,density_fun,yerr=error, fmt='.',capsize=3, label=f'Error {z}', alpha= 0.8)
...
#for analytic method
dir_path = f'/Volumes/Backpack L2/s10 MSc Major Project/Hubble data/HMFcsvdata_(all_at_once)'
...
plt.title(f'Halo Mass Function Comparison in Different Redshift Using Various Models')
